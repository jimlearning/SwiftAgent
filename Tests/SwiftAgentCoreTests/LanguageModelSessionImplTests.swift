...
private struct TestTool: Tool {
    ...
    func call(arguments: Data) async throws -> ToolOutputValue {
...
// MARK: - LanguageModelSessionImplTests

final class LanguageModelSessionImplTests: XCTestCase {
    ...
    func testRespondCompletesTurn() async throws {
        ...
        let response = try await runtime.respond(to: "Hi")
    ...
    func testRespondWithToolCalls() async throws {
        ...
        let response = try await runtime.respond(to: "Use the tool")
        let result = response.transcript
