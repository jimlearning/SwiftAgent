...
final class SQLiteMemoryStoreTests: XCTestCase {
    ...
    func testInitCreatesDatabaseAndSchema() throws {
        ...
        let namespaces = try awaitWithTimeout { try await store.listNamespaces() }
    ...
    func testStoreAndRetrieveString() throws {
        ...
        try awaitWithTimeout {
            try await store.store(key: "k1", namespace: "default", value: "hello")
        }

        let retrieved: String? = try awaitWithTimeout {
            try await store.retrieve(key: "k1", namespace: "default")
        }
    ...
    func testStoreOverwritesExistingKey() throws {
        ...
        let first: String? = try awaitWithTimeout {
            try await store.retrieve(key: "k1", namespace: "default")
        }
        ...
        let second: String? = try awaitWithTimeout {
            try await store.retrieve(key: "k1", namespace: "default")
        }
    ...
    func testRetrieveNonExistentKeyReturnsNil() throws {
        ...
        let retrieved: String? = try awaitWithTimeout {
            try await store.retrieve(key: "nonexistent", namespace: "default")
        }
    ...
    func testSearchFindsMatchingEntries() throws {
        let dbPath = makeTempDBPath()
        let store = try SQLiteMemoryStore(location: dbPath.path)
        ...
        let results = try awaitWithTimeout {
            try await store.search(query: "hello", namespace: "default")
        }
    ...
    func testSearchNoMatchesReturnsEmpty() throws {
        ...
        let results = try awaitWithTimeout {
            try await store.search(query: "nonexistent", namespace: "default")
        }
    ...
    func testSummarizeReturnsCountAndKeyListing() throws {
        let dbPath = makeTempDBPath()
        let store = try SQLiteMemoryStore(location: dbPath.path)
        ...
        let summary = try awaitWithTimeout {
            try await store.summarize(namespace: "default")
        }
    ...
    func testForgetRemovesEntry() throws {
        ...
        // Verify stored
        let before: String? = try awaitWithTimeout {
            try await store.retrieve(key: "k1", namespace: "default")
        }
        ...
        // Verify removed
        let after: String? = try awaitWithTimeout {
            try await store.retrieve(key: "k1", namespace: "default")
        }
        XCTAssertNil(after)
    }
    ...
    func testListNamespacesReturnsDistinctNamespaces() throws {
        let dbPath = makeTempDBPath()
        let store = try SQLiteMemoryStore(location: dbPath.path)
        ...
        let namespaces = try awaitWithTimeout {
            try await store.listNamespaces()
        }
    ...
    func testCodableRoundtripPreservesAllFields() throws {
        ...
        let retrieved: TestRecord? = try awaitWithTimeout {
            try await store.retrieve(key: "testRecord", namespace: "records")
        }
    ...
    func testSchemaMigrationAppliesAllSteps() throws {
        let dbPath = makeTempDBPath()
        ...
        let retrieved1: String? = try awaitWithTimeout {
            try await store1.retrieve(key: "k1", namespace: "default")
        }
        ...
        let retrieved2: String? = try awaitWithTimeout {
            try await store2.retrieve(key: "k1", namespace: "default")
        }
    ...
    func testPersistenceAcrossInstances() throws {
        ...
        let retrieved: String? = try awaitWithTimeout {
            try await store2.retrieve(key: "persistent", namespace: "test")
        }
...
/// Helper to run async throwing code from a synchronous XCTest method.
private func awaitWithTimeout<T>(
    ...
    guard let result = result else {
        throw NSError(domain: "SQLiteMemoryStoreTests", code: -1,
