...
/// Stable trailing text block appended to tool_result user messages so the
/// latest-message prompt-cache breakpoint lands on text, matching Claude Code's
/// tool_result + system-reminder request shape.
public let TOOL_RESULT_CACHE_BREAKPOINT_REMINDER = """
<system-reminder>
PostToolUse context: Tool results above were generated by SwiftAgent. Continue using them as the latest observed tool outputs.
</system-reminder>
"""

public func isToolResultCacheBreakpointReminder(_ text: String) -> Bool {
    text == TOOL_RESULT_CACHE_BREAKPOINT_REMINDER
}

/// Append the stable cache-breakpoint reminder to a tool-result message.
public func appendToolResultCacheBreakpointReminder(to blocks: [ContentBlock]) -> [ContentBlock] {
    ...
    return blocks + [.text(TOOL_RESULT_CACHE_BREAKPOINT_REMINDER)]
}
