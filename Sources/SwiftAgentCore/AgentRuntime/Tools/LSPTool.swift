...
public struct LSPTool: Tool {
    ...
    public func call(arguments: Arguments) async throws -> ToolOutputValue {
        guard isValidOperation(arguments.operation) else {
            return .string("Error: operation must be one of: \(validOperations.joined(separator: ", "))")
        }

        // Resolve the absolute path
        let absolutePath = resolvePath(arguments.filePath, relativeTo: workingDirectory)

        // Try to get symbol at position for context
        let symbol = getSymbolAtPosition(absolutePath, line: arguments.line, character: arguments.character)
