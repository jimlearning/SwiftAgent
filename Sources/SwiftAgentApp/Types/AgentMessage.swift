...
public struct AgentMessage: Identifiable, Equatable {
    ...
    public static func assistantStreaming(id: String = UUID().uuidString) -> AgentMessage {
        AgentMessage(id: id, role: .assistant, blocks: [], isStreaming: true)
    }

    // MARK: - Mutation helpers (each bumps renderToken)

    /// Replace or append text. Uses replacement semantics: finds the last text
    /// block (not just the last block overall) and overwrites it with the
    /// snapshot; otherwise a new block is appended.
    public mutating func appendText(_ text: String) {
        renderToken &+= 1
        if let idx = blocks.lastIndex(where: { if case .text = $0 { return true }; return false }) {
    ...
    /// Replace or append thinking. Uses replacement semantics: finds the last
    /// thinking block (not just the last block overall) and overwrites it with
    /// the snapshot; otherwise a new block is appended.
...
extension AgentMessage {
    ...
    /// Merge split-format messages (thinking + text/tool share message UUID).
    /// Returns one Message per logical message, with all blocks merged.
    private static func mergeSplitMessages(_ messages: [Message]) -> [Message] {
        var groups: [String: (Message, [ContentBlock])] = [:]  // uuid → (base, mergedBlocks)
        var order: [String] = []

        for msg in messages {
            if let (base, existing) = groups[msg.uuid] {
                // Merge: insert thinking blocks first, append rest
                var blocks = existing
                let thinkBlocks = msg.content.filter { if case .thinking = $0 { return true }; return false }
                let otherBlocks = msg.content.filter { if case .thinking = $0 { return false }; return true }
                // Insert thinking at the correct position
                if let lastThinkIdx = blocks.lastIndex(where: { if case .thinking = $0 { return true }; return false }) {
                    blocks.insert(contentsOf: thinkBlocks, at: lastThinkIdx + 1)
                } else {
                    blocks = thinkBlocks + blocks
                }
                blocks.append(contentsOf: otherBlocks)
                // Use latest timestamp, model, stopReason, usage
                groups[msg.uuid] = (base, blocks)
            } else {
                groups[msg.uuid] = (msg, msg.content)
                order.append(msg.uuid)
            }
        }

        return order.compactMap { uuid in
            guard let (base, blocks) = groups[uuid] else { return nil }
            return Message(
                uuid: base.uuid, type: base.type, content: blocks,
                timestamp: base.timestamp, usage: base.usage,
                model: base.model, stopReason: base.stopReason
            )
        }
    }

    public static func fromCore(_ messages: [Message]) -> [AgentMessage] {
        mergeSplitMessages(messages).compactMap { msg in
            switch msg.type {
            case .user:
